    ...
    const usersWithAuthInfo = await Promise.all(
      usersData.map(async (user) => {
        try {
          ...
          if (authError) {
            // Si l'utilisateur n'existe pas dans Supabase Auth, on continue sans erreur
            if (authError.message?.includes('User not found') || authError.code === 'user_not_found') {
             return { ...user, last_login: null };
            }
            console.warn(`Erreur lors de la récupération des infos d'auth pour l'utilisateur ${user.id}:`, authError);
            return { ...user, last_login: null };
          }
          ...
          // Gérer spécifiquement l'erreur "User not found"
